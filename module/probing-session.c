...
static unsigned int disassemble(unsigned char *, unsigned int, int, char *);
...
ssize_t binary_analysis(struct probing_session *p_ses, char __user *u_buf, size_t u_len)
{
	unsigned int i;
	unsigned int b;
	unsigned int e;
	...
	unsigned char *byte;
	unsigned long level_address[LEVELS + 1];
	...
	unsigned char *disassembled;

	unsigned char prefix[LEVELS + 2];

	ssize_t l_len;
	ssize_t b_len = -1;

	if (p_ses && p_ses->idt_entry && p_ses->idt_entry->address)
	{
		if ((line = (unsigned char *) kmalloc(LINE_SIZE, GFP_KERNEL)) == NULL)
			return b_len;

		if ((instruction = (unsigned char *) kmalloc(TOKEN_SIZE, GFP_KERNEL)) == NULL)
		{
			kfree((void *) line);
			return b_len;
		}

		if ((disassembled = (unsigned char *) kmalloc(TOKEN_SIZE, GFP_KERNEL)) == NULL)
		{
			kfree((void *) instruction);
			kfree((void *) line);
			return b_len;
		}
...
#ifdef CONFIG_KALLSYMS
		symbol = (unsigned char *) kmalloc(TOKEN_SIZE, GFP_KERNEL);

		if (strlen(p_ses->idt_entry->name))
			l_len = (ssize_t) scnprintf(line, LINE_SIZE, "[Probing NOP] : " _WHITE_ "Starting binary analysis of the \"%s\" interrupt handler (%d)" _RESET_, p_ses->idt_entry->name, p_ses->idt_select);
		else
#endif
			l_len = (ssize_t) scnprintf(line, LINE_SIZE, "[Probing NOP] : " _WHITE_ "Starting binary analysis of the interrupt handler with no name (%d)" _RESET_, p_ses->idt_select);

		line[l_len++] = '\n';

		if ((b_len + l_len) > u_len)
		{
			kfree((void *) disassembled);
			kfree((void *) instruction);
			kfree((void *) line);
			return b_len;
		}

		copy_to_user((void __user *) &u_buf[b_len], (void *) line, (unsigned long) l_len);

		b_len += l_len;

		for (b=0, level=0; b<=LEVELS; b++)
			level_address[b] = (b == 0) ? p_ses->idt_entry->address : 0;

level_switch:
		for (b=0; b<(level+2); b++)
			prefix[b] = (b < (level+1)) ? '-' : '\0';

follow_jump:
		byte = (unsigned char *) level_address[level];

		for (b=0, count=0; b<SEQUENCE_SIZE; b+=count)
		{
			count = disassemble(&byte[b], SEQUENCE_SIZE - b, ((unsigned int) (((unsigned long) &byte[b]) & 0xffffffffUL)), disassembled);
			...
			l_len = (ssize_t) scnprintf(line, LINE_SIZE, "%s " _YELLOW_ "%08x:" _WHITE_ " %-30s %s" _RESET_, prefix, ((unsigned int) (((unsigned long) &byte[b]) & 0xffffffffUL)), instruction, disassembled);
			...
			if (byte[b] == 0xC2 || byte[b] == 0xC3 || byte[b] == 0xCA || byte[b] == 0xCB || byte[b] == 0xCF) // RET
			{
				...
				l_len = (ssize_t) scnprintf(line, LINE_SIZE, "[Probing NOP] : " _WHITE_ "Binary analysis completed" _RESET_);
			...
			else if (byte[b] == 0xE9 || byte[b] == 0xEA || byte[b] == 0xEB) // JMP
			{
				...
				if (jmp_address)
				{
...
#ifdef CONFIG_KALLSYMS
					if (symbol && (symbol_size = sprint_symbol(symbol, jmp_address)) > 0)
					{
						l_len = (ssize_t) scnprintf(line, LINE_SIZE, "      [" _YELLOW_ "%s" _RESET_ "]", symbol);

						line[l_len++] = '\n';

						if ((b_len + l_len - 1) > u_len)
							break;

						copy_to_user((void __user *) &u_buf[b_len - 1], (void *) line, (unsigned long) l_len);
					...
					{
						l_len = (ssize_t) scnprintf(line, LINE_SIZE, "%s --------: %s", prefix, _WHITE_ "[The destination address of JMP instruction is in a non-mapped page. Inspection is stopped.]" _RESET_);
				...
				else
				{
					if (level)
					{
						l_len = (ssize_t) scnprintf(line, LINE_SIZE, "%s --------: %s", prefix, _WHITE_ "[Unable to resolve the destination address of JMP instruction. Return to the caller function.]" _RESET_);
					...
					{
						l_len = (ssize_t) scnprintf(line, LINE_SIZE, "%s --------: %s", prefix, _WHITE_ "[Unable to resolve the destination address of JMP instruction. Inspection is stopped.]" _RESET_);
			...
			else if (byte[b] == 0x9A || byte[b] == 0xE8) // CALL
			{
				...
				{
#ifdef CONFIG_KALLSYMS
					if (symbol && (symbol_size = sprint_symbol(symbol, call_address)) > 0)
					{
						l_len = (ssize_t) scnprintf(line, LINE_SIZE, "      [" _YELLOW_ "%s" _RESET_ "]", symbol);
					...
					if (check_page_is_valid_get_frame_number(call_address) != -1)
					{
						...
						l_len = (ssize_t) scnprintf(line, LINE_SIZE, "%s --------: %s", prefix, _WHITE_ "[The destination address of CALL instruction is beyond the maximum level. Not expended.]" _RESET_);
					...
					{
						l_len = (ssize_t) scnprintf(line, LINE_SIZE, "%s --------: %s", prefix, _WHITE_ "[The destination address of CALL instruction is in a non-mapped page. Not expanded.]" _RESET_);
				...
				{
					l_len = (ssize_t) scnprintf(line, LINE_SIZE, "%s --------: %s", prefix, _WHITE_ "[Unable to resolve the destination address of CALL instruction. Not expanded.]" _RESET_);
			...
			else if (byte[b] == 0x0F && byte[b+1] == 0x1F)
			{
				if (byte[b+2] == 0x44 && byte[b+3] == 0x00 && byte[b+4] == 0x00) // 5 BYTE NOP
				{
					if (probing_device_append(0, 5, (unsigned long) &byte[b]) == 0)
					{
						l_len = (ssize_t) scnprintf(line, LINE_SIZE, "      [" _BLUE_ "0x%lx" _RESET_ "]", (unsigned long) &byte[b]);
				...
				else if (byte[b+2] == 0x80 && byte[b+3] == 0x00 && byte[b+4] == 0x00 &&
							byte[b+5] == 0x00 && byte[b+6] == 0x00) // 7 BYTE NOP
				{
					if (probing_device_append(0, 7, (unsigned long) &byte[b]) == 0)
					{
						l_len = (ssize_t) scnprintf(line, LINE_SIZE, "      [" _BLUE_ "0x%lx" _RESET_ "]", (unsigned long) &byte[b]);
				...
				else if (byte[b+2] == 0x84 && byte[b+3] == 0x00 && byte[b+4] == 0x00 &&
							byte[b+5] == 0x00 && byte[b+6] == 0x00 && byte[b+7] == 0x00) // 8 BYTE NOP
				{
					if (probing_device_append(0, 8, (unsigned long) &byte[b]) == 0)
					{
						l_len = (ssize_t) scnprintf(line, LINE_SIZE, "      [" _BLUE_ "0x%lx" _RESET_ "]", (unsigned long) &byte[b]);
			...
			else if (byte[b] == 0x66 && byte[b+1] == 0x0F && byte[b+2] == 0x1F)
			{
				if (byte[b+3] == 0x44 && byte[b+4] == 0x00 && byte[b+5] == 0x00) // 6 BYTE NOP
				{
					if (probing_device_append(0, 6, (unsigned long) &byte[b]) == 0)
					{
						l_len = (ssize_t) scnprintf(line, LINE_SIZE, "      [" _BLUE_ "0x%lx" _RESET_ "]", (unsigned long) &byte[b]);
				...
				else if (byte[b+3] == 0x84 && byte[b+4] == 0x00 && byte[b+5] == 0x00 &&
							byte[b+6] == 0x00 && byte[b+7] == 0x00 && byte[b+8] == 0x00) // 9 BYTE NOP
				{
					if (probing_device_append(0, 9, (unsigned long) &byte[b]) == 0)
					{
						l_len = (ssize_t) scnprintf(line, LINE_SIZE, "      [" _BLUE_ "0x%lx" _RESET_ "]", (unsigned long) &byte[b]);
			...
			else if (byte[b] == 0x90 && b >= 4)
			{
				if (byte[b-4] == 0x66 && byte[b-3] == 0x66 && byte[b-2] == 0x66 && byte[b-1] == 0x66) // REP NOP
				{
					if (probing_device_append(1, 5, (unsigned long) &byte[b-4]) == 0)
					{
						l_len = (ssize_t) scnprintf(line, LINE_SIZE, "      [" _BLUE_ "0x%lx" _RESET_ "]", (unsigned long) &byte[b-4]);
...
#ifdef CONFIG_KALLSYMS
		if (symbol) kfree((void *) symbol);
#endif
		kfree((void *) disassembled);
		kfree((void *) instruction);

		l_len = (ssize_t) scnprintf(line, LINE_SIZE, "[Probing NOP] : " _WHITE_ "Listing the available probing functions" _RESET_);
		...
		{
			l_len = (ssize_t) scnprintf(line, LINE_SIZE, _WHITE_ "- Function \"%s\"" _RESET_ "      [" _BLUE_ "0x%lx" _RESET_ "]", get_pf_name(i), get_pf_address(i));
		...
		l_len = (ssize_t) scnprintf(line, LINE_SIZE, "[Probing NOP] : " _WHITE_ "End of list" _RESET_);
...
int insert_probe_function(struct probing_function *p_fun, unsigned long p_fun_addr)
{
	unsigned char *byte = (unsigned char *) p_fun->nop_address;

	unsigned int size = p_fun->nop_size;

	unsigned short old_2_LSB = ((unsigned short) byte[0]) | (((unsigned short) byte[1]) << 8);
	unsigned short new_2_LSB = ((unsigned short) 0xEB) | (((unsigned short) (size - 2)) << 8);
	
	unsigned short *tmp_jmp_addr = (unsigned short *) byte;

	unsigned int call_offset;

	unsigned int old_4_LSB = ((unsigned int) 0xEB) | ((size - 2) << 8) | (((unsigned int) byte[2]) << 16) |
								(((unsigned int) byte[3]) << 24);
	unsigned int new_4_LSB = 0x000000E8;

	unsigned int *tmp_call_addr = (unsigned int *) byte;

	unsigned long next_instr_addr = (unsigned long) &byte[5];
	...
	if (check_page_is_valid_get_frame_number(p_fun_addr) == -1)
		return -1;

	_write_cr0(cr0 & ~X86_CR0_WP);
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,17,0)
	arch_cmpxchg(tmp_jmp_addr, old_2_LSB, new_2_LSB);
#else
	cmpxchg(tmp_jmp_addr, old_2_LSB, new_2_LSB);
#endif
	_write_cr0(cr0);

	asm volatile("rep; nop; mfence" : : );

	if (next_instr_addr > p_fun_addr)
		call_offset = (1U << 31) | ((1U << 31) - (unsigned int) (next_instr_addr - p_fun_addr));
	else
		call_offset = (unsigned int) (p_fun_addr - next_instr_addr);

	_write_cr0(cr0 & ~X86_CR0_WP);
	switch (size-5)
	{
		case 1:
			byte[4] = (unsigned char) ((call_offset & 0xff000000U) >> 24);
			byte[5] = 0x90;
			break;
		case 2:
			byte[4] = (unsigned char) ((call_offset & 0xff000000U) >> 24);
	...
	new_4_LSB |= ((call_offset & 0x000000ffU) << 8) | ((call_offset & 0x0000ff00U) << 8) | ((call_offset & 0x00ff0000U) << 8);

	_write_cr0(cr0 & ~X86_CR0_WP);
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,17,0)
	arch_cmpxchg(tmp_call_addr, old_4_LSB, new_4_LSB);
#else
	cmpxchg(tmp_call_addr, old_4_LSB, new_4_LSB);
#endif
	_write_cr0(cr0);

	pr_info("[Probing NOP] : " _WHITE_ "NOP at address [" _BLUE_ "0x%lx" _WHITE_ "] replaced with CALL to [" _BLUE_ "0x%lx" _WHITE_ "]" _RESET_ "\n", p_fun->nop_address, p_fun_addr);

	return 0;
}

void remove_probe_function(struct probing_function *p_fun)
{
	unsigned char *byte = (unsigned char *) p_fun->nop_address;
	unsigned int size = p_fun->nop_size;
	unsigned int rep = p_fun->nop_rep;
	
	unsigned short *tmp_jmp_addr;

	unsigned short old_2_LSB;
	unsigned short new_2_LSB;

	unsigned int *tmp_call_addr;

	unsigned int old_4_LSB;
	unsigned int new_4_LSB;

	unsigned long cr0 = _read_cr0();

	tmp_jmp_addr = (unsigned short *) byte;
	old_2_LSB = *tmp_jmp_addr;
	new_2_LSB = ((unsigned short) 0xEB) | (((unsigned short) (size - 2)) << 8);
...
#pragma GCC diagnostic pop
	...
	tmp_call_addr = (unsigned int *) byte;
	old_4_LSB = *tmp_call_addr;
	switch (size)
	{
		case 5:
			new_4_LSB = (rep) ? 0x66666666 : 0x00441F0F;
			break;
		case 6:
			new_4_LSB = 0x441F0F66;
			break;
		case 7:
			new_4_LSB = 0x00801F0F;
			break;
		case 8:
			new_4_LSB = 0x00841F0F;
			break;
		case 9:
			new_4_LSB = 0x841F0F66;
			break;
		default:
			return;
	}
	...
	pr_info("[Probing NOP] : " _WHITE_ "Restored NOP at address [" _BLUE_ "0x%lx" _WHITE_ "]" _RESET_ "\n", p_fun->nop_address);
}
